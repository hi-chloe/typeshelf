...
WOFF2 uploads are decompressed with `woff2-encoder/decompress` so cataloging can read the same OS/2 / `post` / `name` tables as TTF/OTF (weight, fixed-pitch, panose, family class). Preview still registers the **original** WOFF2 bytes with `FontFace` — browsers decode those natively.

If decompression fails for a given file (corrupt or unsupported WOFF2), the app keeps today’s fallback: filename-guessed family/style plus a non-blocking warning, and the face still previews when possible.
